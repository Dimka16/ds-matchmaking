...
class ClassicMatchmakingStrategy(MatchmakingStrategy):
    ...
    def handle_player(self, player: Player) -> Optional[Match]:
        ...
        # Find a compatible opponent in same region
        for waiting_player in players:
            ...
            if abs(waiting_player.elo - player.elo) <= self._elo_threshold:
                self._repository.remove_by_id(waiting_player.player_id)
                self._repository.remove_by_id(player.player_id)

                return Match(players=[waiting_player, player], game_mode=GameMode.CLASSIC)
